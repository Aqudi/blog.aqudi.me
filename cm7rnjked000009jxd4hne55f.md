...
하지만 포트포워딩을 하고 접속 테스트를 하려고 공유기 설정을 들어가봤는데, 외부 IP가 `192.168.x.x` 형태로 되어 있더라고요. 포트포워딩을 설정해도 외부에서 접속 자체가 안 되는 상황이었습니다.

이번 글에서는 포트포워딩의 주요 문제점들과 제가 겪었던 CGNAT이라는 환경, 여러 해결법 중 하나인 cloudflare tunnel 사용법까지 정리해보도록 하겠습니다.
