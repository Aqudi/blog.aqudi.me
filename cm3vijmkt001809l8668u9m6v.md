...
* 둘째, 서버리스를 쓸 때는 “서비스 특성 + 사용 패턴”을 함께 봐야 한다는 것이다. Lambda의 실행 모델, 콜드/웜 스타트, Connection Pool, VPC 연결, 타임아웃 등은 서비스마다 영향을 주는 지점이 다르다. “서버리스니까 알아서 잘 해주겠지”가 아니라, 내 워크로드가 그 위에서 어떻게 돌아가는지 한 번은 짚고 넘어가는 게 필요하다.
